...
export function createNoise(seed = 42) {
  ...
  function noise3D(x, y, z) {
    ...
    if (tt > 0) { const gi = permMod12[ii+perm[jj+perm[kk]]]; tt*=tt; n0 = tt*tt*(grad3[gi][0]*x0+grad3[gi][1]*y0+grad3[gi][2]*z0); }
    tt = 0.6 - x1*x1 - y1*y1 - z1*z1;
    if (tt > 0) { const gi = permMod12[ii+i1+perm[jj+j1+perm[kk+k1]]]; tt*=tt; n1 = tt*tt*(grad3[gi][0]*x1+grad3[gi][1]*y1+grad3[gi][2]*z1); }
    tt = 0.6 - x2*x2 - y2*y2 - z2*z2;
    if (tt > 0) { const gi = permMod12[ii+i2+perm[jj+j2+perm[kk+k2]]]; tt*=tt; n2 = tt*tt*(grad3[gi][0]*x2+grad3[gi][1]*y2+grad3[gi][2]*z2); }
    tt = 0.6 - x3*x3 - y3*y3 - z3*z3;
    if (tt > 0) { const gi = permMod12[ii+1+perm[jj+1+perm[kk+1]]]; tt*=tt; n3 = tt*tt*(grad3[gi][0]*x3+grad3[gi][1]*y3+grad3[gi][2]*z3); }
